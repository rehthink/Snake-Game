...
public class CodeSnake extends JPanel implements Runnable {
	...
	public void run() {
		...
		while (Thread.currentThread() == gameThread) {
			...
			try {
				Thread.sleep(Math.max(65-score, 30));
	  ...
	  boolean energyUsed() {
	   ...
	   void moveSnake() {
		   ...
		   void drawSnake(Graphics2D g) {
			      ...
			      g.setColor(energy < 500 ? Color.red : Color.yellow);
			      Point head = snake.get(0);
			   ...
			   void drawStartScreen(Graphics2D g) {
				      g.setColor(Color.red);
				      g.setFont(getFont());
				      g.drawString("SNAKE GAME", 160, 175);
				      g.setColor(Color.white);
				      g.setFont(smallFont);
				      g.drawString("( START )", 280, 230);
				      g.drawString("REHAN AHMED KHAN", 220, 300);
				   ...
				   void getScore(Graphics2D g) {
					      int h = getHeight();
					      g.setFont(smallFont);
					      g.drawString("Game Over", 30,h-30);
					      g.setColor(getForeground());
					      String s = format("High Score: %d    Score: %d", hiScore, score);
					      g.drawString(s, 30, h - 30);
					   } 
	 ...
	 public static void main(String[] args) {
	      SwingUtilities.invokeLater(
	         () -> {
	            ...
	            mainFrame.add(new CodeSnake(), BorderLayout.CENTER);
	            mainFrame.pack();
